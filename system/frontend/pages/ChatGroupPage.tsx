...
const ChatGroupPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'personal' | 'group' | 'contact'>('group');
  ...
  useEffect(() => {
    if (id) {
      setActiveSession({ type: 'group', id });
      setActiveTab('group');
    } else {
  ...
  }, [token, currentUser, loadChatRooms, loadContacts, loadFriends]);
  ...
  const loadMessages = useCallback(async (chatRoomId: number) => {
    ...
    try {
      ...
      const formattedMessages: Message[] = msgs.map((msg: any) => ({
        ...
        groupId: msg.chat_room_id.toString(),
        text: msg.content,
  ...
  // 當切換聊天室時載入訊息
  useEffect(() => {
    if (activeSession && token && activeSession.type === 'group') {
